...
describe("getConverters", () => {
  ...
  test("bold", () => {
    expect(slackConverter.convertToDomString(`*Hello world!*`)).toEqual(
      `<b>Hello world!</b>`,
    );
  });

  test("italic", () => {
    expect(slackConverter.convertToDomString(`_Hello world!_`)).toEqual(
      `<i>Hello world!</i>`,
    );
  });
  ...
  test("link", () => {
    expect(
      slackConverter.convertToDomString(`<https://test.com|Hello world!>`),
    ).toEqual(`<a href="https://test.com">Hello world!</a>`);
  });
  ...
  test("convertToDomString should handle multiple matching tags", () => {
    ...
    expect(slackConverter.convertToDomString(htmlString)).toEqual(
      expectedString,
    );
  });
  ...
  test("convertToCustomMarkLang should return the same string when no matching tags found", () => {
    const htmlString = "Some content";
    expect(slackConverter.convertToCustomMarkLang(htmlString)).toEqual(
      htmlString,
    );
  });
